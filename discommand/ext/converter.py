...
async def boolean(to_convert: Any) -> bool:
	"""Converts to_convert to a boolean if possible
	...
	"""
	try:
		string = str(to_convert)
	except:
		raise ConverterError(f"Failed to convert `{to_convert} into string while converting to boolean...`") # NOTE: Shouldnt happen but i want to play it safe
	if string.lower() == 'true':
		return True
	elif string.lower() == 'false':
		return False
	else:
		raise ConverterError(f"Failed to convert `{string}` to boolean.")

async def integer(to_convert: Any) -> int:
	"""Converts to_convert to an integer if possible
